  ...
  )
{
  ...
  *Completed      = TRUE;
  ...
  while (AnswerSectionNum < DnsHeader->AnswersNum) {
	//UINT32 RecordSize=0;
	//Some of DNS answer name is a full name
    if ((*(UINT8 *)AnswerName & 0xC0) != 0xC0) {
		//Skip the name
		while (*(UINT8 *)AnswerName!= 0) {
			//DEBUG((DEBUG_ERROR, "%c",*(UINT8 *)AnswerName));
	...
	DEBUG((DEBUG_ERROR, "Type : %d\n",AnswerSection->Type));
	DEBUG((DEBUG_ERROR, "TTL : %d\n",AnswerSection->Ttl));
	DEBUG((DEBUG_ERROR, "Class : %d\n",AnswerSection->Class));
	DEBUG((DEBUG_ERROR, "Data Length: %d\n",AnswerSection->DataLength));
    //
    // Check whether the remaining packet length is available or not.
    //
